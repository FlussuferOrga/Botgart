    ...
    command(message : discord.Message, responsible, guild, args) {
       ...
       if(!message.member) {
        ...
        resolveWvWObjective(user_objective).then(([objective, map, obj_id, map_id, obj_type]) => {
            ...
            const cmd_args : string = "\""+exec_time+ "\" ,say #"+current_channel + " \"" + say_text+"\"";
...
/*
    ...
    */
    rescheduleCronjobs() {
        ...
        cl.db.getCronjobs().forEach(cron => {
            ...
            if(!guild) {
                log("error", "TapReminder.js", "I am no longer member of the guild {0} the cronjob with ID {1} was scheduled for. Skipping.".formatUnicorn(cron.guild, cron.id));
            } else {
                let responsible = guild.members.find(m => m.user.id == cron.created_by);
                let job;
                if(!responsible) {
                    log("warn", "TapReminder.js", "Responsible user with ID {0} for cronjob {1} is no longer present in Guild {2}.".formatUnicorn(cron.created_by, cron.id, guild.name));
                } else {
                    job = this.scheduleTapReminder(cron.schedule, responsible.user, guild, mod, args);
                }
                if(!job) {
                    log("error", "TapReminder.js", "Could not reschedule cronjob {0} although it was read from the database.".formatUnicorn(cron.id));
                } else {
                    ...
                    croncount++;
                    log("info", "TapReminder.js", "Rescheduled cronjob {0} of type '{1}'".formatUnicorn(cron.id, cron.command));
                }
            }
        });
        log("info", "TapReminder.js", "Done rescheduling {0} cronjobs.".formatUnicorn(croncount));
        return croncount;
    }
    ...
    * @returns {scheduleJob}
    */
    scheduleTapReminder(time: Date, responsible: discord.User, guild: discord.Guild, cmd: BotgartCommand, args: any): any {
