...
process.on("unhandledRejection", (reason, p) => {
    LOG.log("crit", `Unhandled Rejection!`)
...
if (args.patchall || args.patch) {
    ...
    if (args.patchall) {
        ...
        if (p === undefined) {
            LOG.log("warning", `No patch ${args.patch} could be found to apply/revert.`)
        } else {
    ...
    client.login(config.get().token).then(() => {
        ...
        LOG.log("error", `Error starting up Bot: ${reason}`)
        process.exit(1);
    });
}