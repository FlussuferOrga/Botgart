...
*/
function getResetDay(week, year = new Date().getFullYear(), resetWeekDay = 5) {
    // look for the first reset day of the year
    let resetDay = new Date(year, 0, 1);
    while (resetDay.getDay() != resetWeekDay) {
        resetDay.setDate(resetDay.getDate() + 1);
    }
    const firstResetWeekNumber = getNumberOfWeek(resetDay); // if the first day of the year is either SAT or SUN, the first Friday of the year is in week 2! So check that.
    resetDay.setDate(resetDay.getDate() + (7 * (week - firstResetWeekNumber)));
    return resetDay;
}
exports.getResetDay = getResetDay;
// blatantly stolen from https://gist.github.com/IamSilviu/5899269#gistcomment-2918013
function getNumberOfWeek(today = new Date()) {
...
exports.formatUserPing = formatUserPing;
...
function assignServerRole(member, currentRole, admittedRole) {
    ...
    if (currentRole !== null) {
        // remove currentRole
        member.removeRole(currentRole).then(() => log("info", "Util.js", "Removed role {0} from user {1}".formatUnicorn(currentRole.name, member.displayName)), (err) => log("error", "Util.js", "Error while removing role {0} from user {1}: {2}".formatUnicorn(currentRole.name, member.displayName, err.message)));
    }
    if (admittedRole !== null) {
        // assign admittedRole
        member.addRole(admittedRole).then(() => log("info", "Util.js", "Gave role {0} to user {1}".formatUnicorn(admittedRole.name, member.displayName)), (err) => log("error", "Util.js", "Error while giving role {0} to user {1}: {2}".formatUnicorn(admittedRole.name, member.displayName, err.message)));
    }
    return admittedRole;
}
